...
import id.application.core.data.network.model.products.category.ItemAllProductCategory
...
class CategoriesPagingSource(
    private val repo : ApplicationRepository
):PagingSource<Int, ItemAllProductCategory>(){


    override fun getRefreshKey(state: PagingState<Int, ItemAllProductCategory>): Int? {
        return state.anchorPosition?.let { anchorPosition ->
            state.closestPageToPosition(anchorPosition)?.prevKey?.plus(1)
                ?: state.closestPageToPosition(anchorPosition)?.nextKey?.minus(1)
        }
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ItemAllProductCategory> {
        val currentPage = params.key ?: 1
        return try {
            val response = repo.getAllProductCategories(
                pageItem = currentPage,
                limit = params.loadSize
            )
